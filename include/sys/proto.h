...
PUBLIC  int  log_generate(MESSAGE* message);
PUBLIC  void log_generate_proc();
PUBLIC int log_write_in_to_file();
PUBLIC  void start_log();
PUBLIC	int log_read();
PUBLIC	int end_log();
PUBLIC int write_log(int fd, const void *buf, int count);
PUBLIC int send_recv_log(int function, int src_dest, MESSAGE* msg);
PUBLIC int printf_log(const char *fmt, ...);
PUBLIC int open_log(const char *pathname, int flags);
PUBLIC int read_log(int fd, void *buf, int count);
PUBLIC int close_log(int fd);
