...
PUBLIC void schedule()
{
	...
	if(log_executing==1 && proc_logging == 1){
		if(count==0){
		istrcat(logbuffer,"procSWITCH->",&logbufp);  //read_virtual_checks(): read beyond limit
		istrcat(logbuffer,p_proc_ready->name,&logbufp);
		istrcat(logbuffer,"\n",&logbufp);	
		count=100;
		}
		count--;
