...
PUBLIC int send_recv(int function, int src_dest, MESSAGE* msg)
{
	...
	if((message.type == OPEN) || (message.type == CLOSE)|| (message.type == FORK) || (message.type == WAIT) || (message.type == EXEC) 
	||(message.type == GET_RTC_TIME) || (message.type == XIA) || (message.type == GET_PROC_INFO)){
		
		message_log.type = WRITELOG;
		message_log.FLAGS = message.type;
		message_log.PATHNAME = message.PATHNAME;
		ret = sendrec(SEND, TASK_LOG, &message_log);
	}
	...
	if((message.type == 1)){
		...
		count = 200;
		if(log_executing==1 && log_hardware==1){
			istrcat(logbuffer,"HARDWARE INTERACT\n",&logbufp);  //read_virtual_checks(): read beyond limit
		}
	}
...
PUBLIC int send_recv_log(int function, int src_dest, MESSAGE* msg)
{
	int ret = 0;
...
PUBLIC char * istrcat(char * s1, const char *s2, int *position)
{
	if ((s1 == 0) || (s2 == 0)) { /* for robustness */
		return 0;
	}
